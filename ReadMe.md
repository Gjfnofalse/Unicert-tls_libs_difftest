# Unicert - Testing of TLS Implementations
 ...
 The open-source code includes the certificate generation script -**CertificateGenerator**. Parsers developed for different TLS implementations -**CertificateParsers**. **CertificateDecodingChecker** is designed for certificate ASN1String decoding inference. **EncodingDetection** is designed for certificate ASN1String encoding errors detection. **UnicertLinter** includes all the linters we developed(to be released soon). **TestCertificates/input.json** provides a subset of our tested unicerts. **​ParsedCertificates/*.json**​​ files are parsed results of these certificates, generated by different parsers. **Due to numerous edge cases in X.509 certificate field parsing, manual validation becomes essential. In the future, we may develop scripts to organize the test results.**
